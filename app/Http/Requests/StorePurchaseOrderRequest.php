...
{
    public function authorize(): bool { return true; } // Sẽ check bằng Policy/Gate sau
    ...
    public function rules(): array
    {
        return [
            ...
            'items' => 'required|array|min:1', // Phải có ít nhất 1 item
            'items.*.product_id' => 'required|exists:products,id',
            'items.*.quantity' => 'required|integer|min:1', // Số lượng > 0 [cite: 325]
            'items.*.price' => 'required|numeric|min:0', // Giá >= 0
        ];
    }

    public function messages(): array // Thông báo lỗi tiếng Việt (tùy chọn)
    {
        return [
    ...
    protected function prepareForValidation()
    {
        if ($this->has('items')) {
            ...
            foreach ($items as $index => &$item) {
                $item['position'] = $index + 1; // Add position for error messages
            }
